    ...
    try {
        ...
        data.features.forEach((feature) => {
            try {
                ...
                const name =
                    properties.nom || properties.name || properties.NOM_COM
        ...
        const parisArrondissements = Object.keys(boundaries).filter((code) =>
            code.startsWith("751")
        ).length
        const lyonArrondissements = Object.keys(boundaries).filter((code) =>
            code.startsWith("693")
        ).length
        const marseilleArrondissements = Object.keys(boundaries).filter(
            (code) => code.startsWith("132")
        ).length
        ...
        console.log(
            `  - Marseille: ${marseilleArrondissements} arrondissements`
        )
        ...
        console.log(
            `  - Communes/arrondissements: ${Object.keys(boundaries).length}`
        )
        console.log(
            `  - All Paris arrondissements included: ${parisArrondissements === 20 ? "✅" : "❌"}`
        )
