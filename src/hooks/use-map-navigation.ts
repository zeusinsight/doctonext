...
export function useMapNavigation(
  mapRef: React.RefObject<{ flyTo: (lat: number, lng: number, zoom?: number) => void }>
  ...
  const navigateToDepartment = useCallback((department: DepartmentData) => {
    previousStateRef.current = state
    ...
    if (mapRef.current) {
      const centerLat = (department.bounds.north + department.bounds.south) / 2
      const centerLng = (department.bounds.east + department.bounds.west) / 2

      // Calculate appropriate zoom level based on bounds
      const latDiff = department.bounds.north - department.bounds.south
  ...
  const navigateToCommune = useCallback((commune: CommuneData) => {
    previousStateRef.current = state
