      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          const filtered = result.data.listings
            .filter((listing: MapListing) => listing.id !== listingId)
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Link href={`/map?bounds=${location.latitude-0.1},${location.longitude-0.1},${location.latitude+0.1},${location.longitude+0.1}`}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {nearbyListings.length} annonce{nearbyListings.length > 1 ? "s" : ""} à proximité
                  </span>
                </div>
                <Link href={`/map?region=${encodeURIComponent(location.region)}`}>
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {nearbyListings.slice(0, 3).map((listing) => (
                  ...
                  >
                    ...
                    {listing.specialty && ` • ${listing.specialty}`}
                  </Badge>
                ))}
