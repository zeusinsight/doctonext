...
export function CommuneOverlay({
  ...
  useEffect(() => {
    ...
    const loadCommunes = async () => {
      ...
      try {
        const communeData = await fetchCommunesForDepartment(departmentCode)

        if (!isCancelled) {
          setCommunes(communeData)
          console.log(`CommuneOverlay: Loaded ${communeData.length} communes for department ${departmentCode}`)
        }
      } catch (error) {
        console.error('CommuneOverlay: Error loading communes:', error)
        if (!isCancelled) {
          const errorMessage = error instanceof Error ? error.message : 'Failed to load communes'
      ...
      try {
        ...
        if (commune.geometry.type === 'Polygon') {
          polygon = L.polygon(coordinates as L.LatLngExpression[][], {
            ...
            weight: 1,
            opacity: 0.6,
            fillColor: highlightedCommune === commune.code ? '#10b981' : '#34d399',
            fillOpacity: highlightedCommune === commune.code ? 0.4 : opacity,
        ...
        console.error(`Error creating polygon for commune ${commune.code}:`, error)
      }
    })
  ...
  }, [map, communes, visible, opacity, highlightedCommune, onCommuneClick, onCommuneHover])
