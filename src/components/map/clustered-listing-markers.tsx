...
const createClusterIcon = (count: number, types: string[]): DivIcon => {
  ...
  const color =
    typeColors[dominantType as keyof typeof typeColors] || "#6b7280";
...
const createListingIcon = (
  ...
  const getIconColor = (type: string) => {
    switch (type) {
      ...
      case "replacement":
        return isBoostPlus ? "#1a5ba3" : "#4a8dd9";
      case "collaboration":
...
export function ClusteredListingMarkers({
  listings,
  onMarkerClick,
}: ClusteredListingMarkersProps) {
  const map = useMap();
    ...
    <>
      {clusters.map((cluster) => {
        ...
        if (isCluster && cluster_id && point_count) {
          ...
          const types = clusterListings.map(
            (leaf) => leaf.properties.listing.listingType,
          );
          ...
          return (
            ...
            >
              <Popup maxWidth={300} className="cluster-popup">
                <div className="p-2">
                  <h4 className="mb-2 font-medium">{point_count} annonces</h4>
