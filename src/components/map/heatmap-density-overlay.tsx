...
const generatePolygonPoints = (
    ...
    const adjustedPointCount = Math.max(
        5,
        Math.min(25, Math.floor(pointsCount * populationFactor * areeFactor))
    )
    ...
    for (let i = 0; i < centralPoints; i++) {
        ...
        const offsetLat =
            centroid[0] + Math.cos(angle) * offsetRadius * Math.random()
        const offsetLng =
            centroid[1] + Math.sin(angle) * offsetRadius * Math.random()
    ...
    for (let i = 0; i < scatteredPoints; i++) {
        ...
        const distanceFromCenter = Math.sqrt(
            (lat - centroid[0]) ** 2 + (lng - centroid[1]) ** 2
        )
        const maxDistance = Math.sqrt(latRange ** 2 + lngRange ** 2)
        const distanceFactor = 1 - (distanceFromCenter / maxDistance) * 0.3 // Reduce by up to 30%

        points.push([
            lat,
            lng,
            intensity * distanceFactor * (0.6 + 0.4 * Math.random())
        ])
    }
...
export function HeatmapDensityOverlay({
    ...
    useEffect(() => {
        ...
        const dataToUse =
            useRealData && !densityData?.length
        ...
        return () => {
            if (
                ...
                map.hasLayer(heatmapLayerRef.current)
            ) {
                map.removeLayer(heatmapLayerRef.current)
            }
        }
