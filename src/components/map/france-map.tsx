...
import type { MedicalProfession, ZonageLevel, TownMedicalZones } from "@/lib/services/town-density-types"
import { getZonageColor, getZonageLabel } from "@/lib/services/town-density-types"
...
// Data URLs
const DEPARTMENTS_URL = "https://raw.githubusercontent.com/gregoiredavid/france-geojson/master/departements-version-simplifiee.geojson"
...
const getCommuneUrl = (departmentCode: string) => {
  ...
  if (!folderName) {
    console.error(`No folder mapping found for department code: ${departmentCode}`)
    return null
  }
...
export function FranceMap({ height = "600px", listings = [], className }: FranceMapProps) {
  const [departmentData, setDepartmentData] = useState<FeatureCollection<any> | null>(null)
  const [communeData, setCommuneData] = useState<FeatureCollection<any> | null>(null)
  const [selectedDepartment, setSelectedDepartment] = useState<string | null>(null)
  const [loading, setLoading] = useState(true)
  const [communeLoading, setCommuneLoading] = useState(false)
  const [hoveredDepartment, setHoveredDepartment] = useState<string | null>(null)
  const [loadingDepartmentCode, setLoadingDepartmentCode] = useState<string | null>(null)
  const [selectedProfession, setSelectedProfession] = useState<MedicalProfession>('chirurgiens-dentistes')
  const [parisArrondissements, setParisArrondissements] = useState<FeatureCollection<any> | null>(null)
  ...
  // Debug current state
  console.log("FranceMap render - dataLoaded:", dataLoaded, "parisArrondissements:", !!parisArrondissements, "features:", parisArrondissements?.features?.length)
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const parisResponse = await fetch('/data/arrondissements_paris.geojson')
        if (!parisResponse.ok) {
          throw new Error(`HTTP ${parisResponse.status}: ${parisResponse.statusText}`)
        }
        const parisData = await parisResponse.json()
        setParisArrondissements(parisData)
        console.log("Paris arrondissements data loaded successfully:", parisData?.features?.length, "features")
        setDataLoaded(true)
      } catch (error) {
  ...
  const getDepartmentStyle = useCallback((feature: DepartmentFeature) => {
    ...
    return {
      ...
      opacity: 1,
      color: isLoading ? "#3b82f6" : isHovered ? "#60a5fa" : "#94a3b8",
  ...
  const getCommuneStyle = useCallback((feature: CommuneFeature) => {
  ...
  const onDepartmentClick = useCallback(async (feature: DepartmentFeature, layer: L.Layer) => {
    ...
    if (departmentCode === '75') {
      console.log('Clicked on Paris, checking data:', !!parisArrondissements, parisArrondissements?.features?.length)
    ...
    try {
      ...
      if (!communeUrl) {
        console.error(`No commune data available for department ${departmentCode}`)
        setLoadingDepartmentCode(null)
      ...
      const response = await fetch(communeUrl, { signal: abortController.signal })
      if (!response.ok) {
      ...
      if (error instanceof Error && error.name !== 'AbortError') {
        console.error(`Failed to load commune data for department ${departmentCode}:`, error)
      }
    } finally {
  ...
  const onCommuneClick = useCallback((feature: CommuneFeature) => {
    // For Paris arrondissements, use c_arinsee; for regular communes, use code
    const communeCode = feature.properties.c_arinsee ?
      feature.properties.c_arinsee.toString() :
      feature.properties.code
    ...
    console.log("Clicked commune:", displayName, "Code:", communeCode, zone ? `Zone: ${getZonageLabel(zone)}` : 'No data')
    // You can add more functionality here, like showing listings for this commune
  }, [selectedProfession, densityData])

  // Department event handlers
  const onEachDepartment = useCallback((feature: DepartmentFeature, layer: L.Layer) => {
    layer.on({
  ...
  const onEachCommune = useCallback((feature: CommuneFeature, layer: L.Layer) => {
    // For Paris arrondissements, use c_arinsee; for regular communes, use code
    const communeCode = feature.properties.c_arinsee ?
      feature.properties.c_arinsee.toString() :
      feature.properties.code
  ...
  const handleCitySelect = useCallback(async (city: CityInfo) => {
    if (!map) return
  ...
  if (loading) {
    return (
      <div className={cn("relative bg-gray-100 rounded-lg flex items-center justify-center", className)} style={{ height }}>
  ...
  return (
    <div className={cn("relative", className)} style={{ height }}>
      ...
      <div className="absolute top-4 left-4 z-[1000] flex flex-col gap-2">
        {selectedDepartment && (
          <button
            ...
            className="bg-white/95 backdrop-blur-sm border border-gray-200 shadow-lg rounded-lg px-3 py-2 text-sm font-medium hover:bg-white transition-colors"
          >
            ← Retour à la France
          </button>
        )}
        ...
        <div className="bg-white/95 backdrop-blur-sm border border-gray-200 shadow-lg rounded-lg p-3">
          <label htmlFor="profession-select" className="block text-xs font-medium text-gray-700 mb-2">
            Profession médicale
          </label>
          <select
            id="profession-select"
            value={selectedProfession}
            onChange={(e) => setSelectedProfession(e.target.value as MedicalProfession)}
          ...
          >
            ...
            <option value="masseurs-kinésithérapeutes">Kinésithérapeutes</option>
      ...
      <div className="absolute top-4 right-4 z-[1000] flex flex-col gap-2 max-w-sm">
        {/* City search box */}
        <CitySearchBox
      ...
      <div className="absolute bottom-4 right-4 z-[1000] bg-white/95 backdrop-blur-sm border border-gray-200 shadow-lg rounded-lg p-3">
      ...
      {/* Listings count */}
      <div className="absolute bottom-4 left-4 z-[1000] bg-white/95 backdrop-blur-sm border border-gray-200 shadow-lg rounded-lg px-3 py-2">
