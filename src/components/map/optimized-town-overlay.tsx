...
import {
    ...
    leafletBoundsToViewport,
    type IndexedItem
} from "@/lib/utils/spatial-index"
import {
    MAJOR_CITIES,
    shouldShowCityLabel,
    getCityLabelStyle,
...
export function OptimizedTownOverlay({
        ...
        const initWorker = async () => {
            try {
                ...
                if (
                    ...
                    console.log(
                        "Web Worker initialization skipped for Next.js compatibility"
                    )
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const index = new SpatialIndex(
                    {
                        minLat: 40.0,
                        maxLat: 52.0, // Expanded France bounds
                        minLng: -6.0,
                        maxLng: 11.0
                    },
                    20,
                    6
                ) // Increase maxItems and maxDepth for better granularity
                ...
                if (!isCancelled) {
                    ...
                    console.log(
                        `Indexed ${townsData.size} towns for ${profession}`
                    )
                }
            } catch (error) {
    ...
    const updateVisibleTowns = useCallback(
        async (bounds: L.LatLngBounds) => {
            ...
            if (zoomLevel <= 7) {
                console.log(
                    `Low zoom (${zoomLevel}): using simple viewport filtering for complete coverage`
                )
                ...
                for (const town of townsDataRef.current.values()) {
                    ...
                    if (
                        !(
                            bounds.maxLat < viewport.south - padding ||
                ...
                console.log(
                    `Low zoom: found ${processedTowns.length} towns with simple filtering`
                )
            } else {
                ...
                if (workerRef.current) {
                    try {
                        processedTowns =
                            await workerRef.current.processTownData(
                                new Map(
            ...
            if (zonageFilter?.length) {
                processedTowns = processedTowns.filter((town) =>
                    zonageFilter.includes(town.zonage)
                )
            }
            ...
            if (zoomLevel > 12 && processedTowns.length > 10000) {
                console.warn(
                    `High zoom: limiting ${processedTowns.length} towns to 10000 for performance`
                )
            ...
            console.log(
                `Rendered ${processedTowns.length} towns (zoom: ${zoomLevel})`
            )
        },
