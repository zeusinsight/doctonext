...
export function DepartmentOverlay({
    ...
    const loadDepartments = async () => {
      ...
      try {
        ...
        if (!isCancelled) {
          ...
          console.log(`DepartmentOverlay: Successfully loaded ${departmentData.length} departments`)
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (!map || departments.length === 0) {
      console.log('DepartmentOverlay: Skipping polygon creation - map:', !!map, 'departments:', departments.length)
      return
    }

    console.log(`DepartmentOverlay: Creating polygons for ${departments.length} departments, visible:`, visible)
      ...
      try {
        ...
        if (department.geometry.type === 'Polygon') {
          polygon = L.polygon(coordinates as L.LatLngExpression[][], {
            ...
            weight: 2,
            opacity: 0.8,
            fillColor: highlightedDepartment === department.code ? '#3b82f6' : '#60a5fa',
            fillOpacity: highlightedDepartment === department.code ? 0.6 : opacity,
        ...
        console.error(`Error creating polygon for department ${department.code}:`, error)
      }
    })

    console.log(`DepartmentOverlay: Successfully created ${successCount} department polygons`)

    // Add to map if visible
    if (visible) {
      map.addLayer(layerGroup)
      console.log(`DepartmentOverlay: Added ${successCount} departments to map`)
    } else {
  ...
  }, [map, departments, visible, opacity, highlightedDepartment, onDepartmentClick, onDepartmentHover])
