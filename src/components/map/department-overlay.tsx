    ...
    useEffect(() => {
        ...
        const loadDepartments = async () => {
            ...
            try {
                ...
                console.error(
                    "DepartmentOverlay: Error loading departments:",
                    error
                )
                if (!isCancelled) {
                    const errorMessage =
            ...
            try {
                ...
                if (department.geometry.type === "Polygon") {
                    polygon = L.polygon(coordinates as L.LatLngExpression[][], {
                        ...
                        opacity: 0.8,
                        fillColor:
                            highlightedDepartment === department.code
                                ? "#3b82f6"
                                : "#60a5fa",
                        fillOpacity:
                            highlightedDepartment === department.code
    ...
    useEffect(() => {
        ...
        layerGroupRef.current.eachLayer((layer) => {
            if (layer instanceof L.Polygon) {
                ...
                const departmentCode = className.match(
                    /department-(\d+[AB]?)/
                )?.[1]

                if (departmentCode) {
                    const isHighlighted =
                        highlightedDepartment === departmentCode
