...
async function loadBoundaryData(): Promise<Record<string, CommuneBoundary>> {
    ...
    try {
        ...
        if (typeof window === "undefined") {
            ...
            console.log(
                `Loaded ${Object.keys(boundaryData || {}).length} commune boundaries from split files (Part 1: ${Object.keys(part1Data).length}, Part 2: ${Object.keys(part2Data).length})`
            )
        } else {
            ...
            console.log(
                `Loaded ${Object.keys(boundaryData || {}).length} commune boundaries from API`
            )
        }
 ...
 */
export async function getCommuneBoundaries(
    codes: string[]
...
export async function getCommuneBoundary(
    code: string
): Promise<CommuneWithBoundary | null> {
    const data = await loadBoundaryData()
...
export function isPointInFranceBounds(lat: number, lng: number): boolean {
    ...
    if (
        lat >= metropolitanBounds.south &&
        lat <= metropolitanBounds.north &&
        lng >= metropolitanBounds.west &&
        lng <= metropolitanBounds.east
    ) {
        return true
    }
