...
export const getZonageDensityScore = (zonage: ZonageLevel): number => {
    switch (zonage) {
        case "1_Tres_sous_dotee":
            return 15 // Very under-served (green)
        case "2_Sous_dotee":
            return 35 // Under-served (light green)
        case "3_intermediaire":
            return 55 // Balanced (orange)
        case "4_Tres_dotee":
            return 75 // Well-served (red)
        case "5_Sur_dotee":
            return 95 // Over-served (dark red)
        case "5b_Non_prioritaire":
            return 95 // Non-priority (same as Sur-dotée - dark red)
        default:
            return 0 // Unknown/N/A (gray)
    }
}
