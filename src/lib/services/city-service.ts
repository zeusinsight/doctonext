...
export function searchCities(query: string): CityInfo[] {
    ...
    try {
        ...
        const results = searchCity.byName(
            normalizedQuery,
            50
        ) as FranceCitiesJSCity[]
...
export function getAllCities(): CityInfo[] {
    console.warn(
        "getAllCities() not recommended with france-cities-js due to large dataset size"
    )
    return []
}
