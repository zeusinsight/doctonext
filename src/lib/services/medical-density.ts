...
export async function calculateMedicalDensityByRegion(
    ...
    try {
        ...
        const professionalsByRegion = await db
            ...
            .where(and(eq(users.isVerifiedProfessional, true), ...conditions))
            .groupBy(listingLocations.region)

        // Calculate density scores for each region
        const densityData: RegionDensity[] = professionalsByRegion.map(
            (region) => {
                const population =
                    REGION_POPULATION_DATA[
                        region.region as keyof typeof REGION_POPULATION_DATA
                    ] || 1000000
                const professionalCount = region.professionalCount || 0

                // Calculate professionals per 10,000 inhabitants
                const professionalsPerTenThousand =
                    (professionalCount / population) * 10000
                ...
                const maxExpectedRatio =
                    filters.specialty === "Médecin généraliste" ? 12 : 8
                const densityScore = Math.min(
                    100,
                    (professionalsPerTenThousand / maxExpectedRatio) * 100
                )
                ...
                return {
                    ...
                    bounds: (REGION_BOUNDARIES[
                        region.region as keyof typeof REGION_BOUNDARIES
                    ] || []) as number[][][]
                }
            }
        )
        ...
        Object.keys(REGION_POPULATION_DATA).forEach((region) => {
            if (!regionsWithData.has(region)) {
                densityData.push({
                    ...
                    populationCount:
                        REGION_POPULATION_DATA[
                            region as keyof typeof REGION_POPULATION_DATA
                        ],
                    bounds: (REGION_BOUNDARIES[
                        region as keyof typeof REGION_BOUNDARIES
                    ] || []) as number[][][]
                })
            }
        })
 ...
 */
export async function getMedicalDensityStats(
    filters: MedicalDensityFilters = {}
) {
    try {
        const densityData = await calculateMedicalDensityByRegion(filters)

        const totalProfessionals = densityData.reduce(
            (sum, region) => sum + region.professionalCount,
            0
        )
        const totalPopulation = densityData.reduce(
            (sum, region) => sum + region.populationCount,
            0
        )

        const averageDensity =
            densityData.reduce((sum, region) => sum + region.densityScore, 0) /
            densityData.length

        const underServed = densityData.filter(
            (region) => region.densityScore <= 30
        ).length
        const moderateDensity = densityData.filter(
            (region) => region.densityScore > 30 && region.densityScore <= 70
        ).length
        const overServed = densityData.filter(
            (region) => region.densityScore > 70
        ).length
...
const densityCache = new Map<
    string,
    { data: RegionDensity[]; timestamp: number }
>()
const CACHE_DURATION = 1000 * 60 * 30 // 30 minutes

export async function getCachedMedicalDensity(
    filters: MedicalDensityFilters = {}
) {
