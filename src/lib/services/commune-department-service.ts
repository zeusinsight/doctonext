...
export async function fetchCommunesForDepartment(
    ...
    try {
        ...
        if (cachedData) {
            console.log(
                `Loaded ${cachedData.length} communes for department ${departmentCode} from cache`
            )
            return cachedData
        }

        console.log(
            `Fetching communes for department ${departmentCode} from france-geojson...`
        )
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch communes for department ${departmentCode}: ${response.status}`
            )
        }
        ...
        console.log(
            `Loaded ${communes.length} communes for department ${departmentCode}`
        )
        ...
        console.error(
            `Error fetching communes for department ${departmentCode}:`,
            error
        )
        ...
        try {
            console.log(
                `Fallback: fetching from main communes file for department ${departmentCode}`
            )
            return await fetchCommunesFromMainFile(departmentCode)
        } catch (fallbackError) {
            console.error("Fallback failed:", fallbackError)
            throw new Error(
                `Failed to load communes for department ${departmentCode}`
            )
        }
    }
}
 ...
 */
async function fetchCommunesFromMainFile(
    departmentCode: string
): Promise<CommuneData[]> {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch main communes file: ${response.status}`
            )
        }
        ...
        if (geoJson.features && Array.isArray(geoJson.features)) {
            for (const feature of geoJson.features as CommuneFeature[]) {
                if (
                    ...
                    const communeDepartmentCode = getDepartmentCodeFromCommune(
                        feature.properties.code
                    )
