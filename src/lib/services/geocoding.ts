...
export async function geocodeByCity(
    city: string
    ...
    try {
        ...
        if (!results || results.length === 0)
            return { error: "City not found", code: "NOT_FOUND" }
...
export async function geocodeByPostalCode(
    ...
    try {
        ...
        const postalCodePrefix = cleaned.substring(0, 2)
        ...
        const departmentCoordinates: Record<string, [number, number]> = {
            ...
            "17": [45.75, -0.75], // Charente-Maritime
            "18": [47.0833, 2.4], // Cher
            ...
            "26": [44.7333, 5.05], // Drôme
            "27": [49.0167, 1.15], // Eure
            "28": [48.4667, 1.5], // Eure-et-Loir
            "29": [48.1, -4.1], // Finistère
            "30": [43.8333, 4.35], // Gard
            "31": [43.6044, 1.4442], // Haute-Garonne (Toulouse)
            "32": [43.65, 0.5833], // Gers
            ...
            "47": [44.2, 0.6167], // Lot-et-Garonne
            "48": [44.5183, 3.5014], // Lozère
            "49": [47.4739, -0.55], // Maine-et-Loire
            "50": [49.1167, -1.3], // Manche
            "51": [49.0444, 4.3667], // Marne
            "52": [48.1167, 5.1333], // Haute-Marne
            "53": [48.0667, -0.7667], // Mayenne
            "54": [48.6833, 6.2], // Meurthe-et-Moselle
            "55": [49.1667, 5.3833], // Meuse
            "56": [47.75, -2.75], // Morbihan
            ...
            "68": [47.75, 7.3333], // Haut-Rhin
            "69": [45.764, 4.8357], // Rhône (Lyon)
            ...
            "77": [48.85, 2.95], // Seine-et-Marne
            "78": [48.8, 2.13], // Yvelines
 ...
 */
export async function updateListingCoordinates(
    listingId: string,
    ...
    try {
        ...
        for (const location of listingsToGeocode) {
            ...
            if (
                ...
                geocodeResult.code === "NOT_FOUND"
            ) {
                geocodeResult = await geocodeByPostalCode(
                    location.postalCode,
                    location.city
                )
            }
            ...
            if ("error" in geocodeResult) {
                ...
                results.failed++
                console.error(
                    `Failed to geocode listing ${location.listingId}:`,
                    geocodeResult.error
                )
            } else {
                const updateResult = await updateListingCoordinates(
                    location.listingId,
                    geocodeResult
                )
                if (updateResult.success) {
                    results.success++
                    console.log(
                        `Successfully geocoded listing ${location.listingId}`
                    )
                } else {
                    results.failed++
                }
