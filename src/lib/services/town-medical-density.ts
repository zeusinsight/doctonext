...
import {
    getCommuneBoundaries,
    convertCoordinatesToLeaflet,
    type CommuneWithBoundary
} from "./commune-boundaries"
    ...
    try {
        ...
        for (let i = 0; i < filteredCodes.length; i += batchSize) {
            ...
            if ((i + batchSize) % 500 === 0) {
                console.log(
                    `Processed ${Math.min(i + batchSize, filteredCodes.length)}/${filteredCodes.length} towns...`
                )
            }
        }

        console.log(
            `Successfully loaded ${result.length} towns with boundaries`
        )
        return result
    } catch (error) {
...
export async function getTownDensityStatistics(profession: MedicalProfession) {
    ...
    return {
        ...
        distribution: Array.from(stats.distribution.entries()).map(
            ([zonage, count]) => ({
                zonage,
                count,
 ...
 */
export function convertTownBoundaryToLeaflet(
    town: TownDensityWithBoundary
): L.LatLngExpression[] | L.LatLngExpression[][] {
    return convertCoordinatesToLeaflet(
...
export class TownDensityLoader {
    ...
    async loadTownsProgressively(
        ...
        try {
            const densityData = await getCachedMedicalDensityData()
            const townCodes = Object.keys(densityData).filter(
                (code) => densityData[code].zones[profession] !== undefined
            )
            ...
            console.log(
                `Progressive loading complete: ${this.loadedTowns.size} towns loaded`
            )
            return this.loadedTowns
        } finally {
    ...
    getTownsByZonage(zonage: ZonageLevel): TownDensityWithBoundary[] {
        return Array.from(this.loadedTowns.values()).filter(
            (town) => town.zonage === zonage
        )
    }
