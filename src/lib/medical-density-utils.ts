...
export function getMedicalDensityForField(field: string): RegionDensity[] {
    ...
    Object.entries(medicalDensityData).forEach(([regionName, regionData]) => {
        ...
        if (
            fieldData &&
            REGIONAL_BOUNDS[regionName] &&
            "professionalCount" in fieldData &&
            fieldData.professionalCount !== undefined
        ) {
            const density = calculateDensityScore(
                fieldData.professionalCount,
                fieldData.populationCount
            )
...
export function getMedicalFieldStats(field: string): {
    ...
    const averageDensity =
        totalPopulation > 0 ? (totalProfessionals / totalPopulation) * 10000 : 0
