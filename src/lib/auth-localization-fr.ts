import type { AuthLocalization } from "@daveyplate/better-auth-ui"

export const authLocalizationFr: AuthLocalization = {
    // Account related
    ACCOUNT: "Compte",
    ACCOUNTS: "Comptes",
    ACCOUNTS_DESCRIPTION: "Basculer entre vos comptes actuellement connectés.",
    ACCOUNTS_INSTRUCTIONS: "Se connecter à un compte supplémentaire.",
    ADD_ACCOUNT: "Ajouter un compte",
    DELETE_ACCOUNT: "Supprimer le compte",
    DELETE_ACCOUNT_DESCRIPTION:
        "Supprimez définitivement votre compte et tout son contenu. Cette action est irréversible, veuillez donc continuer avec prudence.",
    DELETE_ACCOUNT_INSTRUCTIONS:
        "Veuillez confirmer la suppression de votre compte. Cette action est irréversible, veuillez donc continuer avec prudence.",
    DELETE_ACCOUNT_VERIFY:
        "Veuillez vérifier votre e-mail pour confirmer la suppression de votre compte.",
    DELETE_ACCOUNT_SUCCESS: "Votre compte a été supprimé.",

    // Avatar
    AVATAR: "Avatar",
    AVATAR_DESCRIPTION:
        "Cliquez sur l'avatar pour télécharger un avatar personnalisé depuis vos fichiers.",
    AVATAR_INSTRUCTIONS: "Un avatar est facultatif mais fortement recommandé.",
    DELETE_AVATAR: "Supprimer l'avatar",
    UPLOAD_AVATAR: "Télécharger l'avatar",

    // Authentication
    SIGN_IN: "Connexion",
    SIGN_IN_ACTION: "Se connecter",
    SIGN_IN_DESCRIPTION:
        "Entrez votre e-mail ci-dessous pour vous connecter à votre compte",
    SIGN_IN_USERNAME_DESCRIPTION:
        "Entrez votre nom d'utilisateur ou e-mail pour vous connecter à votre compte",
    SIGN_IN_WITH: "Se connecter avec",
    SIGN_OUT: "Déconnexion",
    SIGN_UP: "Inscription",
    SIGN_UP_ACTION: "Créer un compte",
    SIGN_UP_DESCRIPTION: "Entrez vos informations pour créer un compte",
    SIGN_UP_EMAIL: "Vérifiez votre e-mail pour le lien de vérification.",
    ALREADY_HAVE_AN_ACCOUNT: "Vous avez déjà un compte ?",
    DONT_HAVE_AN_ACCOUNT: "Vous n'avez pas de compte ?",

    // Email
    EMAIL: "E-mail",
    EMAIL_DESCRIPTION:
        "Entrez l'adresse e-mail que vous souhaitez utiliser pour vous connecter.",
    EMAIL_INSTRUCTIONS: "Veuillez entrer une adresse e-mail valide.",
    EMAIL_IS_THE_SAME: "L'e-mail est identique",
    EMAIL_PLACEHOLDER: "nom@exemple.com",
    EMAIL_REQUIRED: "L'adresse e-mail est requise",
    EMAIL_VERIFY_CHANGE:
        "Veuillez vérifier votre e-mail pour confirmer le changement.",
    EMAIL_VERIFICATION:
        "Veuillez vérifier votre e-mail pour le lien de vérification.",
    VERIFY_YOUR_EMAIL: "Vérifiez votre e-mail",
    VERIFY_YOUR_EMAIL_DESCRIPTION:
        "Veuillez vérifier votre adresse e-mail. Vérifiez votre boîte de réception pour l'e-mail de vérification. Si vous n'avez pas reçu l'e-mail, cliquez sur le bouton ci-dessous pour renvoyer.",
    RESEND_VERIFICATION_EMAIL: "Renvoyer l'e-mail de vérification",

    // Name
    NAME: "Nom",
    NAME_DESCRIPTION:
        "Veuillez entrer votre nom complet ou un nom d'affichage.",
    NAME_INSTRUCTIONS: "Veuillez utiliser 32 caractères au maximum.",
    NAME_PLACEHOLDER: "Nom",

    // Password
    PASSWORD: "Mot de passe",
    PASSWORD_PLACEHOLDER: "Mot de passe",
    PASSWORD_REQUIRED: "Le mot de passe est requis",
    PASSWORDS_DO_NOT_MATCH: "Les mots de passe ne correspondent pas",
    CURRENT_PASSWORD: "Mot de passe actuel",
    CURRENT_PASSWORD_PLACEHOLDER: "Mot de passe actuel",
    NEW_PASSWORD: "Nouveau mot de passe",
    NEW_PASSWORD_PLACEHOLDER: "Nouveau mot de passe",
    NEW_PASSWORD_REQUIRED: "Le nouveau mot de passe est requis",
    CONFIRM_PASSWORD: "Confirmer le mot de passe",
    CONFIRM_PASSWORD_PLACEHOLDER: "Confirmer le mot de passe",
    CONFIRM_PASSWORD_REQUIRED: "La confirmation du mot de passe est requise",
    CHANGE_PASSWORD: "Changer le mot de passe",
    CHANGE_PASSWORD_DESCRIPTION:
        "Entrez votre mot de passe actuel et un nouveau mot de passe.",
    CHANGE_PASSWORD_INSTRUCTIONS: "Veuillez utiliser au minimum 8 caractères.",
    CHANGE_PASSWORD_SUCCESS: "Votre mot de passe a été changé.",
    FORGOT_PASSWORD: "Mot de passe oublié",
    FORGOT_PASSWORD_ACTION: "Envoyer le lien de réinitialisation",
    FORGOT_PASSWORD_DESCRIPTION:
        "Entrez votre e-mail pour réinitialiser votre mot de passe",
    FORGOT_PASSWORD_EMAIL:
        "Vérifiez votre e-mail pour le lien de réinitialisation du mot de passe.",
    FORGOT_PASSWORD_LINK: "Mot de passe oublié ?",
    RESET_PASSWORD: "Réinitialiser le mot de passe",
    RESET_PASSWORD_ACTION: "Enregistrer le nouveau mot de passe",
    RESET_PASSWORD_DESCRIPTION: "Entrez votre nouveau mot de passe ci-dessous",
    RESET_PASSWORD_SUCCESS: "Mot de passe réinitialisé avec succès",
    SET_PASSWORD: "Définir le mot de passe",
    SET_PASSWORD_DESCRIPTION:
        "Cliquez sur le bouton ci-dessous pour recevoir un e-mail afin de configurer un mot de passe pour votre compte.",

    // Sessions
    SESSIONS: "Sessions",
    SESSIONS_DESCRIPTION: "Gérez vos sessions actives et révoquez l'accès.",
    CURRENT_SESSION: "Session actuelle",
    SESSION_NOT_FRESH:
        "Votre session n'est pas récente. Veuillez vous reconnecter.",

    // Settings
    SETTINGS: "Paramètres",
    SECURITY: "Sécurité",

    // Two Factor
    TWO_FACTOR: "Double authentification",
    TWO_FACTOR_CARD_DESCRIPTION:
        "Ajoutez une couche de sécurité supplémentaire à votre compte.",
    TWO_FACTOR_ENABLE_INSTRUCTIONS:
        "Veuillez entrer votre mot de passe pour activer la 2FA.",
    TWO_FACTOR_DISABLE_INSTRUCTIONS:
        "Veuillez entrer votre mot de passe pour désactiver la 2FA.",
    TWO_FACTOR_ENABLED: "L'authentification à deux facteurs a été activée",
    TWO_FACTOR_DISABLED: "L'authentification à deux facteurs a été désactivée",
    ENABLE_TWO_FACTOR: "Activer la double authentification",
    DISABLE_TWO_FACTOR: "Désactiver la double authentification",

    // Actions
    SAVE: "Enregistrer",
    CANCEL: "Annuler",
    CONTINUE: "Continuer",
    DELETE: "Supprimer",
    DONE: "Terminé",
    UPLOAD: "Télécharger",
    GO_BACK: "Retour",

    // Providers
    PROVIDERS: "Fournisseurs",
    PROVIDERS_DESCRIPTION: "Connectez votre compte avec un service tiers.",
    OR_CONTINUE_WITH: "Ou continuer avec",

    // Other
    REMEMBER_ME: "Se souvenir de moi",
    UPDATED_SUCCESSFULLY: "mis à jour avec succès",
    REQUEST_FAILED: "La demande a échoué",
    IS_REQUIRED: "est requis",
    IS_INVALID: "est invalide",
    IS_THE_SAME: "est identique",

    // Organizations
    ORGANIZATION: "Organisation",
    ORGANIZATIONS: "Organisations",
    ORGANIZATIONS_DESCRIPTION: "Gérez vos organisations et vos adhésions.",
    CREATE_ORGANIZATION: "Créer une organisation",
    DELETE_ORGANIZATION: "Supprimer l'organisation",
    LEAVE_ORGANIZATION: "Quitter l'organisation",
    MANAGE_ORGANIZATION: "Gérer l'organisation",

    // API Keys
    API_KEYS: "Clés API",
    API_KEYS_DESCRIPTION: "Gérez vos clés API pour un accès sécurisé.",
    API_KEYS_INSTRUCTIONS:
        "Générez des clés API pour accéder à votre compte par programmation.",
    CREATE_API_KEY: "Créer une clé API",
    DELETE_API_KEY: "Supprimer la clé API",
    API_KEY: "Clé API",

    // Passkeys
    PASSKEY: "Clé d'accès",
    PASSKEYS: "Clés d'accès",
    PASSKEYS_DESCRIPTION: "Gérez vos clés d'accès pour un accès sécurisé.",
    PASSKEYS_INSTRUCTIONS:
        "Accédez en toute sécurité à votre compte sans mot de passe.",
    ADD_PASSKEY: "Ajouter une clé d'accès",

    // Magic Link
    MAGIC_LINK: "Lien magique",
    MAGIC_LINK_ACTION: "Envoyer le lien magique",
    MAGIC_LINK_DESCRIPTION: "Entrez votre e-mail pour recevoir un lien magique",
    MAGIC_LINK_EMAIL: "Vérifiez votre e-mail pour le lien magique",

    // Email OTP
    EMAIL_OTP: "Code e-mail",
    EMAIL_OTP_SEND_ACTION: "Envoyer le code",
    EMAIL_OTP_VERIFY_ACTION: "Vérifier le code",
    EMAIL_OTP_DESCRIPTION: "Entrez votre e-mail pour recevoir un code",
    EMAIL_OTP_VERIFICATION_SENT:
        "Veuillez vérifier votre e-mail pour le code de vérification.",

    // Backup codes
    BACKUP_CODE: "Code de sauvegarde",
    BACKUP_CODES: "Codes de sauvegarde",
    BACKUP_CODES_DESCRIPTION:
        "Enregistrez ces codes de sauvegarde dans un endroit sûr. Vous pouvez les utiliser pour accéder à votre compte si vous perdez votre méthode d'authentification à deux facteurs.",
    BACKUP_CODE_PLACEHOLDER: "Code de sauvegarde",
    BACKUP_CODE_REQUIRED: "Le code de sauvegarde est requis",

    // Copy
    COPIED_TO_CLIPBOARD: "Copié dans le presse-papiers",
    COPY_TO_CLIPBOARD: "Copier dans le presse-papiers",
    COPY_ALL_CODES: "Copier tous les codes",

    // Misc
    PERSONAL_ACCOUNT: "Compte personnel",
    USER: "Utilisateur",
    SWITCH_ACCOUNT: "Changer de compte",
    TRUST_DEVICE: "Faire confiance à cet appareil",
    NEVER_EXPIRES: "N'expire jamais",
    EXPIRES: "Expire",
    NO_EXPIRATION: "Pas d'expiration",
    REVOKE: "Révoquer",
    UNLINK: "Délier",
    LINK: "Lier",
    ACCEPT: "Accepter",
    REJECT: "Rejeter",
    SEND_INVITATION: "Envoyer l'invitation",
    ROLE: "Rôle",
    SELECT_ROLE: "Sélectionner un rôle",
    ADMIN: "Administrateur",
    MEMBER: "Membre",
    GUEST: "Invité",
    OWNER: "Propriétaire",
    MEMBERS: "Membres",

    // Terms
    PRIVACY_POLICY: "Politique de confidentialité",
    TERMS_OF_SERVICE: "Conditions d'utilisation",
    BY_CONTINUING_YOU_AGREE: "En continuant, vous acceptez les",
    PROTECTED_BY_RECAPTCHA: "Ce site est protégé par reCAPTCHA."
}
