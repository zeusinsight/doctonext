...
export async function getPublicListings(filters?: {
  ...
  try {
    ...
    if (filters?.collaborationType) {
      conditions.push(
        eq(collaborationDetails.collaborationType, filters.collaborationType),
      );
    }
...
export async function getListingCountsBySpecialty() {
  try {
    const counts = await db
      ...
      .groupBy(listings.specialty);

    // Convert array to object for easier lookup
    const countsMap: Record<string, number> = {};
    counts.forEach((item) => {
      if (item.specialty) {
        countsMap[item.specialty] = item.count;
      }
    });
