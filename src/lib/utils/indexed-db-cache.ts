...
class IndexedDBCache {
  ...
  async getTownBoundaries(profession: string, maxAge = 30 * 60 * 1000): Promise<any | null> {
    return null
  }

  async storeSimplifiedBoundaries(profession: string, zoomLevel: number, data: any): Promise<void> {
    // Disabled
  }

  async getSimplifiedBoundaries(profession: string, zoomLevel: number, maxAge = 60 * 60 * 1000): Promise<any | null> {
    return null
  }
  ...
  async getListings(filters: any, maxAge = 5 * 60 * 1000): Promise<any | null> {
    return null
  }
  ...
  async getStorageSize(): Promise<{ estimated: number; quota: number } | null> {
