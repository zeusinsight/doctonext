...
export function useTownData(profession: MedicalProfession, limit = 50000) {
  return useQuery({
    ...
    queryFn: async (): Promise<TownDataResponse> => {
      const response = await fetch(`/api/map/town-density?profession=${profession}&limit=${limit}`)

      if (!response.ok) {
        throw new Error(`Failed to load town data: ${response.statusText}`)
      }
...
export function usePrefetchTownData() {
  ...
  const prefetchTownData = (profession: MedicalProfession, limit = 50000) => {
    queryClient.prefetchQuery({
      ...
      queryFn: async (): Promise<TownDataResponse> => {
        const response = await fetch(`/api/map/town-density?profession=${profession}&limit=${limit}`)
        if (!response.ok) throw new Error('Failed to prefetch town data')
...
export function useTownStats(profession: MedicalProfession) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/map/town-density?profession=${profession}&stats=true`)
...
export function useTownsByZonage(profession: MedicalProfession, zonage: ZonageLevel, limit = 1000) {
  return useQuery({
    queryKey: ['towns-by-zonage', profession, zonage, limit],
    queryFn: async () => {
      const response = await fetch(`/api/map/town-density?profession=${profession}&zonage=${zonage}&limit=${limit}`)
