...
export function useListings() {
  return useQuery({
    ...
    queryFn: async (): Promise<ListingDataResponse> => {
      ...
      if (!response.ok) {
        throw new Error(`Failed to load listings: ${response.statusText}`)
      }
...
export function useMapListings(params: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MapListingDataResponse> => {
      ...
      if (!response.ok) {
        throw new Error(`Failed to load map listings: ${response.statusText}`)
      }
  ...
  const prefetchMapListings = (params: {
    ...
    queryClient.prefetchQuery({
      ...
      queryFn: async () => {
        const url = `/api/map/listings${queryString ? `?${queryString}` : ""}`
        const response = await fetch(url)
        if (!response.ok) throw new Error('Failed to prefetch map listings')
        return response.json()
      },
      staleTime: 60 * 1000,
    })
...
export function useSearchListings(query: string, filters: any, debounceMs = 300) {
  ...
  return useQuery({
    ...
    enabled: debouncedQuery.trim().length > 0 || Object.keys(filters).length > 0,
