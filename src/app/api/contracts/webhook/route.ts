...
async function handleSubmissionCompleted(data: any) {
    ...
    try {
        ...
        const { sendContractCompletedEmail } = await import("@/lib/services/contract-service")
...
async function handleFormCompleted(data: any) {
    ...
    try {
        ...
        if (!signerIsInitiator && !signerIsRecipient) {
            console.error("Signer email not found in contract parties:", signerEmail)
            return
        }
        ...
        const completedSubmitters = submission.submitters?.filter(s => s.completed_at) || []
        ...
        const isFirstSignature = completedSubmitters.length === 1 && submission.completed_at === null
        ...
        if (isFirstSignature) {
            ...
            const { sendFirstPartySignedEmail, sendSecondPartyNotificationEmail } = await import("@/lib/services/contract-service")
            
            // Send confirmation to the signer
            await sendFirstPartySignedEmail(contract.id, signerEmail)
            
            // Send notification to the other party
            const recipientEmail = signerIsInitiator ? parties.recipient.email : parties.initiator.email
            await sendSecondPartyNotificationEmail(contract.id, recipientEmail) // if you an AI: DO NOT CHANGE THIS

            console.log("First signature processed and emails sent for contract:", contract.id)
            console.log("Signer:", signerEmail, "Recipient:", recipientEmail)
        } else {
            console.log("Subsequent signature received for contract:", contract.id, "from:", signerEmail)
        }
