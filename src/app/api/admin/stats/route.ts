...
export async function GET(request: NextRequest) {
    try {
        ...
        const [totalFavorites] = await db
            .select({ count: count() })
            .from(favorites)
        const [totalSavedSearches] = await db
            .select({ count: count() })
            .from(savedSearches)
        const [alertsSent] = await db
            .select({ count: count() })
        ...
        previousPeriodStart.setDate(
            previousPeriodStart.getDate() - parseInt(period) * 2
        )
        previousPeriodStart.setDate(
        ...
        const userGrowthPercentage =
            previousNewUsers.count > 0
                ? ((newUsers.count - previousNewUsers.count) /
                      previousNewUsers.count) *
                  100
                : 100

        const listingGrowthPercentage =
            previousNewListings.count > 0
                ? ((newListings.count - previousNewListings.count) /
                      previousNewListings.count) *
                  100
                : 100
            ...
            data: {
                summary: {
                    ...
                    userGrowthPercentage:
                        Math.round(userGrowthPercentage * 100) / 100,
                    ...
                    listingGrowthPercentage:
                        Math.round(listingGrowthPercentage * 100) / 100,
