...
import { listings, listingLocations, users, transferDetails, replacementDetails } from "@/database/schema"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters = {
      listingType: searchParams.get("type") as "transfer" | "replacement" | "collaboration" | undefined,
    ...
    if (filters.bounds) {
      try {
        const [lat1, lng1, lat2, lng2] = filters.bounds.split(",").map(parseFloat)
    ...
    const transferPrices = listingIds.length > 0 ? await db
      .select({
        listingId: transferDetails.listingId,
        salePrice: transferDetails.salePrice
      })
      .from(transferDetails)
      .where(sql`${transferDetails.listingId} IN (${sql.join(listingIds.map(id => sql`${id}`), sql`,`)})`)
    : []

    const replacementPrices = listingIds.length > 0 ? await db
      .select({
        listingId: replacementDetails.listingId,
        dailyRate: replacementDetails.dailyRate
      })
      .from(replacementDetails)
      .where(sql`${replacementDetails.listingId} IN (${sql.join(listingIds.map(id => sql`${id}`), sql`,`)})`)
    : []

    // Create lookup maps for pricing data
    const transferPriceMap = new Map(transferPrices.map(t => [t.listingId, t.salePrice]))
    const replacementPriceMap = new Map(replacementPrices.map(r => [r.listingId, r.dailyRate]))

    // Combine data and filter by price if needed
    const mapListings: MapListing[] = baseListings
      .map(listing => {
        const salePrice = listing.listingType === "transfer" ? transferPriceMap.get(listing.id) : undefined
        const dailyRate = listing.listingType === "replacement" ? replacementPriceMap.get(listing.id) : undefined
        
        return {
          id: listing.id,
          title: listing.title,
          listingType: listing.listingType as "transfer" | "replacement" | "collaboration",
