...
import {
    getTownMedicalDensityWithBoundaries,
    getTownDensityStatistics,
    getTownsByZonageLevel
} from "@/lib/services/town-medical-density"
import type {
    MedicalProfession,
    ZonageLevel
} from "@/lib/services/town-density"
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (zonageLevel) {
            const towns = await getTownsByZonageLevel(
                profession,
                zonageLevel,
                maxResults
            )
        ...
        return NextResponse.json({
            ...
            data: {
                ...
                meta: {
                    ...
                    note:
                        maxResults > actualLimit
                            ? `Results limited to ${actualLimit} for performance`
                            : undefined
                }
            }
        })
